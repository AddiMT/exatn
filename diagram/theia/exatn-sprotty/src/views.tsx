/** @jsx svg */
import { svg } from 'snabbdom-jsx';

import { VNode } from "snabbdom/vnode";
import { Point, PolylineEdgeView, RenderingContext, SNode,   toDegrees, IView, SPort,EmptyView, CircularNodeView } from "sprotty";
import { injectable } from 'inversify';
import { TensorEdge } from './model';

@injectable()
export class PolylineArrowEdgeView extends PolylineEdgeView {

    protected renderAdditionals(edge: TensorEdge, segments: Point[], context: RenderingContext): VNode[] {
        const p1 = segments[segments.length - 2]
        const p2 = segments[segments.length - 1]
        return [
            <path class-sprotty-edge-arrow={true} d="M 6,-3 L 0,0 L 6,3 Z"
                  transform={`rotate(${this.angle(p2, p1)} ${p2.x} ${p2.y}) translate(${p2.x} ${p2.y})`}/>
        ]
    }


    // protected renderDanglingEdge(message: string, edge: SEdge, context: RenderingContext): VNode {
    //     return <text class-sprotty-edge-dangling={true} d="M 6,-3 L 0,0 L 6,3 Z"/>;
    // }

    angle(x0: Point, x1: Point): number {
        return toDegrees(Math.atan2(x1.y - x0.y, x1.x - x0.x))
    }
}

@injectable()
export class TriangleButtonView implements IView {
    render(model: SPort, context: RenderingContext, args?: object): VNode {
        return <path class-sprotty-button={true} d="M 0,0 L 8,4 L 0,8 Z" />
    }
}

/**
 * A very simple example node consisting of a plain circle.
 */
@injectable()
export class CircleNodeView extends CircularNodeView {
    render(node: SNode, context: RenderingContext): VNode {
        const radius = this.getRadius(node);
        return <g>
            <circle class-sprotty-node={true} class-selected={node.selected} class-blank={node.id==='node0'} r={radius} cx={radius} cy={radius}></circle>           
            <text x={radius} y={radius + 7} class-sprotty-text={true}>{node.id.substr(4)}</text>
        </g>;
    }

    protected getRadius(node: SNode) {
        return 40;
    }
    //class-blank={node.id==='node0'}
}


// @injectable()
// export class CircleNodeView extends ShapeView {
//     render(node: SNode, context: RenderingContext): VNode | undefined {
//         if (!this.isVisible(node, context)) {
//             return undefined;
//         }
//         const radius = this.getRadius(node);
//         return <g>
//             <circle class-sprotty-node={true}
//                     class-selected={node.selected}
//                     class-mouseover={node.hoverFeedback}
//                     r={radius} cx={radius} cy={radius}>
//             </circle>
//             <text x={radius} y={radius + 7} class-sprotty-text={true}>{node.id.substr(4)}</text>
//         </g>;
//     }

//     protected getRadius(node: SNode): number {
//         //const d = Math.min(node.size.width, node.size.height);
//         //return d > 0 ? d / 2 : 0;
//         return 40;
//     }
// }

/**
 * A very simple example node consisting of a plain circle.
 */
@injectable()
export class DefaultCircleNodeView extends EmptyView  {
   
}

export class BlankCircleNodeView extends EmptyView  {
   
}


// @injectable()
// export class BlankCircleNodeView extends ShapeView {
//     render(node: SNode, context: RenderingContext): VNode | undefined {
//         if (!this.isVisible(node, context)) {
//             return undefined;
//         }
//         const radius = this.getRadius(node);
//         return <g>
//             <circle class-sprotty-node-blank={true}
//                     class-selected={node.selected}
//                     class-mouseover={node.hoverFeedback}
//                     r={radius} cx={radius} cy={radius}>
//             </circle>
//             <text x={radius} y={radius + 3} class-sprotty-text={true}>{node.id.substr(4)}</text>
//         </g>;
//     }

//     protected getRadius(node: SNode): number {
//        // const d = Math.min(node.size.width, node.size.height);
//        // return d > 0 ? d / 2 : 0;
//        return 10;
//     }
// }


